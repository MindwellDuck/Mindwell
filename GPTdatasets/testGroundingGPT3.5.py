...
output_folder = '/Users/ahmedgouda/Desktop/groundingdrive/SamplesForTestingModels/TestGroundingGPT3.5'
output_file = os.path.join(output_folder, 'grounded_dataset_GPT.csv')
...
# Instruction to GPT for modifying text subtly
system_message = """
...
Your task is to take an existing human-written thought and make **minimal, subtle** modifications to make it exhibit the given cognitive distortion.
Ensure that:
...
def extract_wrapped_text(text):
    ...
    if match:
        ...
        # Fallback: return the original text if no wrapping found
        return text.strip()

# Function to modify the original text to introduce a cognitive distortion in order
def modify_text(original_text, distortion):
    ...
    user_message = (
        ...
        f"You should **add 1 to 3 sentences** at the end in the same style as the original to very subtly display the distortion and add context, but the core writing should feel unaltered in style. "
        f"Ensure that the final text feels **natural and unpolished**, as if it were written by the original person expressing their thoughts.\n\n"
