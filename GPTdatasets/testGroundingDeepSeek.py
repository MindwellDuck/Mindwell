...
# File paths
input_file = '/Users/ahmedgouda/Desktop/groundingdrive/SamplesForTestingModels/TestingSampleDataset1k.csv'
output_folder = '/Users/ahmedgouda/Desktop/groundingdrive/SamplesForTestingModels/TestGroundingDeepSeek'
output_file = os.path.join(output_folder, 'grounded_dataset_DeepSeek.csv')
...
# System instructions for rewriting text
system_message = """
...
IMPORTANT: When providing the final modified text, wrap it in double square brackets. For example, if the final text is "example", return it as [example].
"""

# Function to extract text between double square brackets
def extract_wrapped_text(text):
    match = re.search(r"\[\[(.*?)\]\]", text, re.DOTALL)
    if match:
        return match.group(1).strip()
    else:
        # Fallback: return the original text if no wrapping is found
        return text.strip()

# Function to modify the original text using DeepSeek via Ollama
def modify_text(original_text, distortion):
    ...
    user_message = (
        ...
        f"Do not show me your thinking process. Just respond with the full final modified text.\n"
        f"Wrap the final modified text in double square brackets (i.e., [[ and ]]).\n\n"
        f"Here is the text to modify:\n"
        f"'{original_text}'\n\n"
        f"Modified text:"
    )
    
    # Combine system instructions and user message into one prompt
    prompt = system_message + "\n" + user_message
    
    # Use DeepSeek via Ollama. Make sure the model identifier matches your pulled model.
    model_identifier = "deepseek-r1:7b"
    command = ["ollama", "run", model_identifier, prompt]
    
    try:
        # Set timeout to 60 seconds to prevent infinite loops
        result = subprocess.run(command, capture_output=True, text=True, timeout=60)
        if result.returncode != 0:
            print(f"Error modifying text for distortion {distortion}:")
            print(result.stderr)
            return original_text
        
        full_response = result.stdout.strip()
        # Extract only the text within the double square brackets
        modified_text = extract_wrapped_text(full_response)
        return modified_text
    except subprocess.TimeoutExpired:
        print(f"Timeout expired for distortion {distortion}. Skipping this entry.")
        return original_text
    except Exception as e:
        print(f"Exception modifying text for distortion {distortion}: {e}")
...
for index, row in df.iterrows():
    ...
    time.sleep(1)  # Pause to prevent overwhelming the model
...
save_dataset(df_combined, output_file)
print(f"Resumed processing completed. Dataset saved to {output_file}")
